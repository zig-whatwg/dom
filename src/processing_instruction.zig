...
test "ProcessingInstruction.create - basic creation" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "xml-stylesheet", "type=\"text/css\"");
    defer pi.prototype.prototype.release();

    try testing.expectEqual(.processing_instruction, pi.prototype.prototype.node_type);
    try testing.expectEqualStrings("xml-stylesheet", pi.target);
    try testing.expectEqualStrings("type=\"text/css\"", pi.prototype.data);
}

test "ProcessingInstruction.create - node name is target" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "xml", "version=\"1.0\"");
    defer pi.prototype.prototype.release();

    const name = pi.prototype.prototype.nodeName();
    try testing.expectEqualStrings("xml", name);
}

test "ProcessingInstruction - node value access" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "app", "setting=value");
    defer pi.prototype.prototype.release();

    const value = pi.prototype.prototype.nodeValue();
    try testing.expect(value != null);
    try testing.expectEqualStrings("setting=value", value.?);
}

test "ProcessingInstruction - set node value" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "app", "initial");
    defer pi.prototype.prototype.release();

    try pi.prototype.prototype.setNodeValue("modified");
    try testing.expectEqualStrings("modified", pi.prototype.data);

    const value = pi.prototype.prototype.nodeValue();
    try testing.expectEqualStrings("modified", value.?);
}

test "ProcessingInstruction - inherits Text methods" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "app", "Hello");
    defer pi.prototype.prototype.release();

    // Test Text.appendData
    try pi.prototype.appendData(" World");
    try testing.expectEqualStrings("Hello World", pi.prototype.data);

    // Test Text.length
    try testing.expectEqual(@as(usize, 11), pi.prototype.length());

    // Test Text.substringData
    const sub = try pi.prototype.substringData(allocator, 0, 5);
    defer allocator.free(sub);
    try testing.expectEqualStrings("Hello", sub);
}

test "ProcessingInstruction - clone node" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "xml", "version=\"1.0\"");
    defer pi.prototype.prototype.release();

    const cloned_node = try pi.prototype.prototype.cloneNode(false);
    defer cloned_node.release();

    try testing.expectEqual(.processing_instruction, cloned_node.node_type);

    const cloned_text: *Text = @fieldParentPtr("prototype", cloned_node);
    const cloned_pi: *ProcessingInstruction = @fieldParentPtr("prototype", cloned_text);
    try testing.expectEqualStrings("xml", cloned_pi.target);
    try testing.expectEqualStrings("version=\"1.0\"", cloned_pi.prototype.data);

    // Verify they are separate copies
    try pi.prototype.appendData(" encoding=\"UTF-8\"");
    try testing.expectEqualStrings("version=\"1.0\" encoding=\"UTF-8\"", pi.prototype.data);
    try testing.expectEqualStrings("version=\"1.0\"", cloned_pi.prototype.data);
}
...
test "ProcessingInstruction - target is readonly" {
    ...
    const pi = try ProcessingInstruction.create(allocator, "xml-stylesheet", "href=\"style.css\"");
    defer pi.prototype.prototype.release();

    // Target is readonly - verify it doesn't change
    try testing.expectEqualStrings("xml-stylesheet", pi.target);

    // Data can be modified
    try pi.prototype.appendData(" type=\"text/css\"");
    try testing.expectEqualStrings("href=\"style.css\" type=\"text/css\"", pi.prototype.data);
...
test "ProcessingInstruction - empty data" {
    ...
    const pi = try ProcessingInstruction.create(allocator, "xml", "");
    defer pi.prototype.prototype.release();

    try testing.expectEqual(@as(usize, 0), pi.prototype.length());
    try testing.expectEqualStrings("", pi.prototype.data);
    try testing.expectEqualStrings("xml", pi.target);
}

test "ProcessingInstruction - XML declaration example" {
    const allocator = testing.allocator;

    const pi = try ProcessingInstruction.create(allocator, "xml", "version=\"1.0\" encoding=\"UTF-8\"");
    defer pi.prototype.prototype.release();

    try testing.expectEqualStrings("xml", pi.target);
    try testing.expectEqualStrings("version=\"1.0\" encoding=\"UTF-8\"", pi.prototype.data);
    try testing.expectEqualStrings("xml", pi.prototype.prototype.nodeName());
}
