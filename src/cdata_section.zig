...
test "CDATASection.create - basic creation" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "test content");
    defer cdata.prototype.prototype.release();

    try testing.expectEqual(.cdata_section, cdata.prototype.prototype.node_type);
    try testing.expectEqualStrings("test content", cdata.prototype.data);
}

test "CDATASection.create - node name is #cdata-section" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "content");
    defer cdata.prototype.prototype.release();

    const name = cdata.prototype.prototype.nodeName();
    try testing.expectEqualStrings("#cdata-section", name);
}

test "CDATASection - node value access" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "test data");
    defer cdata.prototype.prototype.release();

    const value = cdata.prototype.prototype.nodeValue();
    try testing.expect(value != null);
    try testing.expectEqualStrings("test data", value.?);
}

test "CDATASection - set node value" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "initial");
    defer cdata.prototype.prototype.release();

    try cdata.prototype.prototype.setNodeValue("modified");
    try testing.expectEqualStrings("modified", cdata.prototype.data);

    const value = cdata.prototype.prototype.nodeValue();
    try testing.expectEqualStrings("modified", value.?);
}

test "CDATASection - inherits Text methods" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "Hello");
    defer cdata.prototype.prototype.release();

    // Test Text.appendData
    try cdata.prototype.appendData(" World");
    try testing.expectEqualStrings("Hello World", cdata.prototype.data);

    // Test Text.length
    try testing.expectEqual(@as(usize, 11), cdata.prototype.length());

    // Test Text.substringData
    const sub = try cdata.prototype.substringData(allocator, 0, 5);
    defer allocator.free(sub);
    try testing.expectEqualStrings("Hello", sub);
}

test "CDATASection - clone node" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "original");
    defer cdata.prototype.prototype.release();

    const cloned_node = try cdata.prototype.prototype.cloneNode(false);
    defer cloned_node.release();

    try testing.expectEqual(.cdata_section, cloned_node.node_type);

    const cloned_text: *Text = @fieldParentPtr("prototype", cloned_node);
    const cloned_cdata: *CDATASection = @fieldParentPtr("prototype", cloned_text);
    try testing.expectEqualStrings("original", cloned_cdata.prototype.data);

    // Verify they are separate copies
    try cdata.prototype.appendData(" modified");
    try testing.expectEqualStrings("original modified", cdata.prototype.data);
    try testing.expectEqualStrings("original", cloned_cdata.prototype.data);
}

test "CDATASection - XML special characters" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "if (x < y && y > z) { }");
    defer cdata.prototype.prototype.release();

    try testing.expectEqualStrings("if (x < y && y > z) { }", cdata.prototype.data);

    // Verify special characters are preserved
    try testing.expect(std.mem.indexOf(u8, cdata.prototype.data, "<") != null);
    try testing.expect(std.mem.indexOf(u8, cdata.prototype.data, ">") != null);
    try testing.expect(std.mem.indexOf(u8, cdata.prototype.data, "&&") != null);
}

test "CDATASection - empty content" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "");
    defer cdata.prototype.prototype.release();

    try testing.expectEqual(@as(usize, 0), cdata.prototype.length());
    try testing.expectEqualStrings("", cdata.prototype.data);
}

test "CDATASection - splitText creates CDATA sections" {
    const allocator = testing.allocator;

    const cdata = try CDATASection.create(allocator, "Hello World");
    defer cdata.prototype.prototype.release();

    const second = try cdata.splitText(6);
    defer second.prototype.prototype.release();

    // Both should be CDATA sections
    try testing.expectEqual(.cdata_section, cdata.prototype.prototype.node_type);
    try testing.expectEqual(.cdata_section, second.prototype.prototype.node_type);

    try testing.expectEqualStrings("Hello ", cdata.prototype.data);
    try testing.expectEqualStrings("World", second.prototype.data);
}
