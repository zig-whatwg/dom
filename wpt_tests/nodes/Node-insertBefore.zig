...
test "Calling insertBefore with an inclusive ancestor must throw HIERARCHY_REQUEST_ERR" {
    ...
    _ = try body.prototype.appendChild(&child.node);

    // Step 2: Cannot insert ancestor into descendant
    const result = body.prototype.insertBefore(&body.node, &child.node);
    try std.testing.expectError(error.HierarchyRequestError, result);
}
...
test "insertBefore with null reference node appends at end" {
    ...
    _ = try parent.prototype.insertBefore(&child1.node, null);
    _ = try parent.prototype.insertBefore(&child2.node, null);

    // Both should be appended
    try std.testing.expect(parent.prototype.first_child == &child1.node);
    try std.testing.expect(parent.prototype.last_child == &child2.node);
}
...
test "insertBefore inserts before reference node" {
    ...
    _ = try parent.prototype.insertBefore(&child2.node, &child3.node);

    // Order should be: child1, child2, child3
    try std.testing.expect(parent.prototype.first_child == &child1.node);
    try std.testing.expect(child1.prototype.next_sibling == &child2.node);
    try std.testing.expect(child2.prototype.next_sibling == &child3.node);
    try std.testing.expect(parent.prototype.last_child == &child3.node);
}
...
test "insertBefore moves node from old parent" {
    ...
    _ = try parent1.prototype.appendChild(&child.node);
    try std.testing.expect(child.prototype.parent_node == &parent1.node);

    // Insert into parent2
    _ = try parent2.prototype.insertBefore(&child.node, null);

    // Should be removed from parent1 and added to parent2
    try std.testing.expect(child.prototype.parent_node == &parent2.node);
    try std.testing.expect(parent1.prototype.first_child == null);
    try std.testing.expect(parent2.prototype.first_child == &child.node);
}
